...
class TestStrategyCondition:
    ...
    def test_strategy_condition_creation(self):
        ...
        condition = StrategyCondition(
            name="低PE", field="pe_ratio", operator="<", value=20, weight=0.3, description="市盈率低于20"
        )
    ...
    def test_strategy_condition_defaults(self):
        ...
        condition = StrategyCondition(name="test", field="test_field", operator=">", value=10)
...
class TestStrategyConfig:
    ...
    def test_strategy_config_creation(self):
        """测试策略配置创建"""
        from asset_lens.strategy.engine import StrategyCondition, StrategyConfig

        buy_conditions = [StrategyCondition(name="低PE", field="pe_ratio", operator="<", value=20)]
        sell_conditions = [StrategyCondition(name="高PE", field="pe_ratio", operator=">", value=40)]

        config = StrategyConfig(
            name="test_strategy", description="测试策略", buy_conditions=buy_conditions, sell_conditions=sell_conditions
        )
