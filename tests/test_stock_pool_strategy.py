...
class TestAddStocksByStrategy:
    ...
    def test_auto_remove_low_score(self, pool):
        ...
        with patch("asset_lens.strategy.engine.strategy_engine") as mock_engine:
            ...
            result = pool.add_stocks_by_strategy(
                "value", stocks, min_score=60.0, max_stocks=10, auto_remove_low_score=True
            )
    ...
    def test_max_stocks_limit(self, pool):
        stocks = [{"code": f"stock{i}", "name": f"Stock {i}", "strategy_score": 80 + i} for i in range(5)]
        with patch("asset_lens.strategy.engine.strategy_engine") as mock_engine:
            mock_engine.screen_stocks.return_value = stocks
...
class TestGetStrategyTopStocks:
    def test_top_stocks(self, pool):
        pool.add_stock("600519", "贵州茅台", 0.0, "watching", "test")
        pool.positions["600519"].selected_history = [{"date": "2025-01-01", "strategy": "value", "score": 85}]
        pool.add_stock("000858", "五粮液", 0.0, "watching", "test")
        pool.positions["000858"].selected_history = [{"date": "2025-01-01", "strategy": "value", "score": 70}]
    ...
    def test_no_matching_strategy(self, pool):
        pool.add_stock("600519", "贵州茅台", 0.0, "watching", "test")
        pool.positions["600519"].selected_history = [{"date": "2025-01-01", "strategy": "momentum", "score": 85}]
