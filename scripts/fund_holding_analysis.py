                ...
                with opener.open(req, timeout=15) as response:
                    ...
                    for line in text.strip().split("\n"):
                        ...
                        if code and len(code) == 6:
                            result[code] = {"price": price, "change": change, "name": name}
        ...
        with urllib.request.urlopen(req, timeout=10) as response:
            ...
            for match in matches1:
                ...
                if stock_code and stock_code not in seen_stocks:
                    seen_stocks[stock_code] = {"code": stock_code, "name": stock_name, "ratio": ratio}
            ...
            for match in matches7:
                ...
                if stock_code and stock_code not in seen_stocks:
                    seen_stocks[stock_code] = {"code": stock_code, "name": stock_name, "ratio": ratio, "market": "海外"}
        ...
        with urllib.request.urlopen(req, timeout=10) as response:
            ...
            for match in etf_matches:
                ...
                if etf_code not in seen_stocks:
                    seen_stocks[etf_code] = {"code": etf_code, "name": etf_name, "ratio": ratio, "market": "ETF"}
...
def analyze_my_fund_holdings(min_stock_ratio: float = 20.0, exclude_bond: bool = True):
    ...
    for fund in fund_products:
        ...
        if holdings_data and holdings_data.get("holdings"):
            ...
            if total_stock_ratio >= min_stock_ratio:
                ...
                console.print(
                    f"    [green]✅ 获取到 {len(holdings_data['holdings'])} 只持仓股票，股票仓位: {total_stock_ratio:.2f}%[/green]"
                )
            else:
    ...
    sorted_stocks = sorted(stock_summary.items(), key=lambda x: x[1]["total_ratio"], reverse=True)
    ...
    if sorted_industries:
        ...
        for i, (industry, ratio) in enumerate(sorted_industries[:15], 1):
            ...
            table.add_row(str(i), industry, f"{ratio:.2f}%", f"{percent:.1f}%")
    ...
    with open(f"{output_dir}/my_stock_watchlist.txt", "w", encoding="utf-8") as f:
        ...
        for i, (code, data) in enumerate(sorted_stocks[:50], 1):
            ...
            f.write(
                f"{i:2d}. {data['name']}({code}): 持仓比例 {data['total_ratio']:.2f}%, 行业: {industry}, 现价: {price}, 涨跌: {change}%\n"
            )
...
def list_funds():
    ...
    for i, fund in enumerate(fund_products, 1):
        table.add_row(
            str(i), fund.get("code", ""), fund.get("name", ""), fund.get("type", ""), fund.get("platform", "")
        )
...
if __name__ == "__main__":
    ...
    if args.detail:
        ...
        analyze_my_fund_holdings(
            min_stock_ratio=args.min_ratio if not args.all else 0, exclude_bond=not args.include_bond and not args.all
        )
    else:
        analyze_my_fund_holdings(
            min_stock_ratio=args.min_ratio if not args.all else 0, exclude_bond=not args.include_bond and not args.all
