...
def load_high_risk_funds():
    ...
    if os.path.exists(data_path):
        ...
        # 计算占比
        df['占比(%)'] = df['初始金额'] / total_amount * 100
...
def generate_high_risk_report(results):
    ...
    if medium_high_funds:
        ...
        # 只显示前5个
        report.extend(f"   • {fund['名称']} ({fund['代码']}) - {fund['占比(%)']:.2f}%" for fund in medium_high_funds[:5])
    ...
    if medium_funds:
        ...
        # 只显示占比高的
        report.extend(f"   • {fund['名称']} ({fund['代码']}) - {fund['占比(%)']:.2f}%" for fund in medium_funds if fund['占比(%)'] > 0.5)
        report.append("")
