...
def balance_samples(X: pd.DataFrame, y: pd.Series, method: str = 'oversample') -> tuple:
    ...
    if method == 'oversample':
        ...
        logger.info("📊 平衡后样本分布: %s", Counter(y_balanced))
...
def cross_validate_model(X: pd.DataFrame, y: pd.Series, n_splits: int = 3) -> dict:
    ...
    for fold, (train_idx, val_idx) in enumerate(tscv.split(X)):
        ...
        if len(y_val.unique()) < 2:
            logger.info("   Fold %s: 跳过 (只有一个类别)", fold + 1)
            continue
...
def main():
    """主函数"""
    logger.info("=" * 60)
    logger.info("      ML 准确率优化 v2 (目标: 72% → 80%)")
    logger.info("=" * 60)

    start_time = time.time()

    X, y, _stocks_data = prepare_enhanced_data(days=500, balance=True)

    cv_results = cross_validate_model(X, y, n_splits=3)
    logger.info("📊 交叉验证结果:")
    logger.info(f"   准确率: {cv_results['accuracy_mean']:.2%} ± {cv_results['accuracy_std']:.2%}")
    logger.info(f"   AUC:    {cv_results['auc_mean']:.4f} ± {cv_results['auc_std']:.4f}")
    logger.info(f"   F1:     {cv_results['f1_mean']:.2%}")
