...
class EnhancedFeatureEngineerV2(FeatureEngineer):
    ...
    def calculate_all_features(self, df: pd.DataFrame, code: str = None) -> pd.DataFrame:
        """计算所有特征"""
...
def train_ensemble_blend(X_train, y_train, X_test, y_test):
    ...
    if HAS_XGBOOST:
        ...
        print(f"   XGBoost: Acc={xgb_metrics['accuracy']:.2%}, AUC={xgb_metrics['auc']:.4f}")
    ...
    if HAS_LIGHTGBM:
        ...
        print(f"   LightGBM: Acc={lgb_metrics['accuracy']:.2%}, AUC={lgb_metrics['auc']:.4f}")
...
def main():
    ...
    if 'blend_weights' in metrics:
        print(f"   融合权重: {metrics['blend_weights']}")
