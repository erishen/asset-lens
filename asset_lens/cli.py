...
def analyze(data_mode, output_format, data_path):
    ...
    for product in portfolio.products:
        if product.transaction_records:
            ...
            if (
                ...
                and product.current_amount
                and product.investment_days
            ):
    ...
    report_data["products"] = [p.to_dict() for p in portfolio.products]
...
@click.option(
    "--target-mode", type=click.Choice(["sample", "real"]), required=True, help="目标数据模式"
)
...
@cli.command()
@click.option(
    "--currency", type=click.Choice(["USD", "HKD"]), required=True, help="货币类型"
)
...
def show_asset_summary(data_mode):
    ...
    if data_mode == "real":
        ...
        csv_file = data_dir / "money_csv_20260226" / "备份-表格 1.csv"
    else:
        data_dir = config.project_root / "data" / "sample_data"
        csv_file = data_dir / "工作表 1-表格 1.csv"
    
    if not csv_file.exists():
        click.echo(f"❌ 资产汇总文件不存在: {csv_file}")
        click.echo(f"💡 提示: 请确保 {data_dir.name} 目录中有相应的数据文件")
        return
    ...
    try:
        ...
        for summary in summaries[-10:]:  # 显示最近10条
            ...
            click.echo(f"  微信: ¥{summary.wechat_amount:,.2f}")
            click.echo(f"  中金: ¥{summary.zhongjin_amount:,.2f}")
            click.echo(f"  支付宝: ¥{summary.alipay_amount:,.2f}")
            click.echo(f"  富途: ¥{summary.futu_amount:,.2f}")
            click.echo(f"  招商: ¥{summary.zhaoshang_amount:,.2f}")
            click.echo(f"  港招: ¥{summary.gangzhao_amount:,.2f}")
            click.echo(f"  交通: ¥{summary.jiaotong_amount:,.2f}")
            click.echo(f"  浦发: ¥{summary.pufa_amount:,.2f}")
            click.echo(f"  建设: ¥{summary.jianshe_amount:,.2f}")
            click.echo(f"  中信: ¥{summary.zhongxin_amount:,.2f}")
            click.echo(f"  民生: ¥{summary.minsheng_amount:,.2f}")
            click.echo(f"  工商: ¥{summary.gongshang_amount:,.2f}")
            click.echo(f"  中银: ¥{summary.zhongyin_amount:,.2f}")
...
def show_exchange_rate_history(data_mode):
    """显示汇率历史（工作表 2-表格 1.csv）"""
    from .data.exchange_rate_parser import ExchangeRateParser
    
    if data_mode == "real":
        data_dir = config.project_root / "data" / "real"
        csv_file = data_dir / "money_csv_20260226" / "工作表 2-表格 1.csv"
    else:
        data_dir = config.project_root / "data" / "sample_data"
        csv_file = data_dir / "工作表 2-表格 1.csv"
...
def show_sell_records(data_mode):
    ...
    if data_mode == "real":
        data_dir = config.project_root / "data" / "real"
        csv_file = data_dir / "money_csv_20260226" / "卖出记录-表格 1.csv"
    else:
        data_dir = config.project_root / "data" / "sample_data"
    ...
    try:
        ...
        if output_format == "console":
            ...
            with open(output_file, "w", encoding="utf-8-sig", newline="") as f:
                fieldnames = [
                    "汇总日期", "微信金额", "中金金额", "支付宝金额", "富途金额",
                    "招商金额", "港招金额", "交通金额", "浦发金额", "建设金额",
                    "中信金额", "民生金额", "工商金额", "中银金额", "信用卡金额",
                    "京东白条金额", "抖音月付金额", "多多后付金额", "总金额",
                    "美元汇率", "港元汇率", "黄金金额", "兑换美元金额", "兑换港元金额",
                    "兑换黄金金额", "上证指数", "沪深300", "中证500", "纳指100",
                    "标普500", "恐慌VXX", "美联基利率", "房产总价", "收益率"
                ]
                ...
                for summary in summaries:
                    writer.writerow({
                        ...
                        "微信金额": str(summary.wechat_amount),
                        "中金金额": str(summary.zhongjin_amount),
                        "支付宝金额": str(summary.alipay_amount),
                        "富途金额": str(summary.futu_amount),
                        "招商金额": str(summary.zhaoshang_amount),
                        "港招金额": str(summary.gangzhao_amount),
                        "交通金额": str(summary.jiaotong_amount),
                        "浦发金额": str(summary.pufa_amount),
                        "建设金额": str(summary.jianshe_amount),
                        "中信金额": str(summary.zhongxin_amount),
                        "民生金额": str(summary.minsheng_amount),
                        "工商金额": str(summary.gongshang_amount),
                        "中银金额": str(summary.zhongyin_amount),
...
def export_sell_records(data_mode, output_format):
    ...
    try:
        ...
        if output_format == "console":
            ...
            with open(output_file, "w", encoding="utf-8-sig", newline="") as f:
                fieldnames = [
                    "卖出日期", "名称", "风险等级", "到期时间", "是否滚动",
                    "开始日期", "初始金额", "收益金额", "收益率", "结束日期",
                ...
                for record in records:
                    writer.writerow({
                        ...
                        "到期时间": record.maturity_date.strftime('%Y-%m-%d') if record.maturity_date else None,
                        ...
                        "到账日期": record.to_account_date.strftime('%Y-%m-%d') if record.to_account_date else None,
...
def init():
    ...
    click.echo("   2. 运行 'asset-lens init-sample' 创建示例数据")
    click.echo("   3. 运行 'asset-lens analyze' 开始分析")
...
def estimate_pnl(data_mode, weekly):
    ...
    try:
        ...
        if result['details']:
            ...
            table.add_column("指数", style="blue")
...
def analyze_sold(data_mode):
    ...
    try:
        ...
        if result['details']:
            ...
            table = Table(title="\n已卖出投资明细")
            table.add_column("产品名称", style="cyan")
            table.add_column("卖出日期", style="green")
            table.add_column("初始金额", justify="right", style="yellow")
            table.add_column("收益金额", justify="right")
            table.add_column("收益率", justify="right")
            table.add_column("持有天数", justify="right")
            table.add_column("年化收益率", justify="right")
            ...
            for detail in result['details'][:20]:  # 只显示前20个
                ...
                table.add_row(
                    detail.name[:20],
                    detail.sell_date.isoformat(),
                    f"¥{detail.initial_amount:,.0f}",
                    f"¥{detail.profit_amount:,.2f}",
                    f"{detail.return_rate:.2f}%",
                    str(detail.holding_days),
                    f"{detail.annualized_return:.2f}%",
                )
...
@cli.command()
@click.option("--api", type=click.Choice(["finnhub", "alphavantage"]), default="alphavantage", help="选择海外市场数据 API (alphavantage: 完整历史数据, finnhub: 仅实时数据)")
...
@click.option("--data-mode", type=click.Choice(["sample", "real"]), help="数据模式")
@click.option("--risk-preference", type=click.Choice(["conservative", "balanced", "aggressive"]), default="balanced", help="风险偏好")
def ai_analyze(data_mode, risk_preference):
    ...
    try:
        ...
        portfolio_data = {
            ...
            'overall_return_rate': float(portfolio.overall_return_rate) if portfolio.overall_return_rate else 0,
        ...
        for p in portfolio.products:
            ...
            portfolio_data['risk_distribution'][risk_name]['total_value'] += float(p.current_amount or 0)
        
        for risk_name in portfolio_data['risk_distribution']:
            portfolio_data['risk_distribution'][risk_name]['percentage'] = (
                portfolio_data['risk_distribution'][risk_name]['total_value'] / portfolio_data['total_value'] * 100
            )
        ...
        for p in portfolio.products:
            ...
            portfolio_data['type_distribution'][type_name]['total_value'] += float(p.current_amount or 0)
        
        for type_name in portfolio_data['type_distribution']:
            portfolio_data['type_distribution'][type_name]['percentage'] = (
                portfolio_data['type_distribution'][type_name]['total_value'] / portfolio_data['total_value'] * 100
            )
        ...
        # 综合评分
        score_color = "green" if result['score'] >= 60 else "yellow" if result['score'] >= 40 else "red"
        console.print(Panel(f"综合评分: {result['score']} 分 ({result['score_level']})", 
                          title="📊 综合评分", border_style=score_color))
...
def portfolio_metrics_cmd(data_mode):
    ...
    try:
        ...
        # 生成模拟的日收益率序列
        # 基于整体收益率和平均投资天数计算
        avg_investment_days = sum(p.investment_days or 0 for p in portfolio.products if p.investment_days) / max(1, len([p for p in portfolio.products if p.investment_days]))
