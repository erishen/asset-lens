...
class ComparisonAnalyzer:
    ...
    def compare_periods(
        ...
        for name in all_names:
            ...
            # 获取金额
            amount_before = product_before.current_amount if product_before and product_before.current_amount else Decimal("0")
            amount_after = product_after.current_amount if product_after and product_after.current_amount else Decimal("0")
        ...
        total_return_rate = (total_change / total_amount_before * Decimal("100")) if total_amount_before != 0 else Decimal("0")
    ...
    def analyze_by_type(
        ...
        for type_key, stats in type_stats.items():
            if stats["total_initial"] > 0:
                stats["return_rate"] = (stats["total_profit"] / stats["total_initial"] * Decimal("100"))
            else:
                stats["return_rate"] = Decimal("0")
    ...
    def generate_comparison_report(
        ...
        improving_rate = (Decimal(str(improving_count)) / Decimal(str(total_count)) * Decimal("100")) if total_count > 0 else Decimal("0")
        deteriorating_rate = (Decimal(str(deteriorating_count)) / Decimal(str(total_count)) * Decimal("100")) if total_count > 0 else Decimal("0")
