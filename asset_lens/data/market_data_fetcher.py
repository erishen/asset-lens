...
from ..data.market_index import MarketIndex, IndexPerformance, MarketIndexCache
...
class MarketDataFetcher:
        ...
        try:
            ...
            with urlopen(request, timeout=30) as response:
                ...
                if len(sorted_dates) > 1:
                    ...
                    prev_close = float(time_series[prev_date].get("4. close", 0))
    ...
    def _calculate_period_performance(self, time_series: Dict, sorted_dates: List[str]) -> Dict[str, Any]:
        try:
            if not sorted_dates:
                return {}
            ...
            for i, date in enumerate(sorted_dates[:22]):
                if i == 21:
                    price = float(time_series[date].get("4. close", 0))
                    monthly_change = ((latest_price - price) / price * 100) if price > 0 else 0
    ...
    def _estimate_technical_status(self, history: List[Dict], current_price: float) -> Dict[str, Any]:
        try:
            if len(history) < 5:
                return {}
    ...
    def fetch_foreign_index_alphavantage(self, symbol: str, api_key: str) -> Optional[Dict[str, Any]]:
        try:
