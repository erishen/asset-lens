...
class ReportGenerator:
    ...
    def print_console_report(self, report: dict[str, Any]) -> None:
        ...
        self._print_report_header(console, report)
        self._print_exchange_rates(console, report)
        self._print_sold_analysis(console, report)
        self._print_time_group_analysis(console, report)
        self._print_portfolio_summary(console, report)
        self._print_type_distribution(console, report)
        self._print_special_bonds(console, report)
        ...
        self._print_efficiency(console, report)

        console.print("\n" + "=" * 60)

    def _print_report_header(self, console: Console, report: dict[str, Any]) -> None:
        """打印报告标题"""
        console.print("")
        title = Panel(
    ...
    def _print_sold_analysis(self, console: Console, report: dict[str, Any]) -> None:
        """打印已实现收益分析"""
        sold_analysis = report.get("sold_investment_analysis")
        if not sold_analysis:
            return
    ...
    def _print_time_group_analysis(self, console: Console, report: dict[str, Any]) -> None:
        """打印时间分组分析"""
        time_analysis = report.get("time_group_analysis", {})
        if not time_analysis.get("groups"):
            return
    ...
    def _print_special_bonds(self, console: Console, report: dict[str, Any]) -> None:
        """打印特别国债"""
    ...
    def _print_efficiency(self, console: Console, report: dict[str, Any]) -> None:
        """打印投资效率"""
