...
from ..data.models import InvestmentProduct, InvestmentType, Portfolio, RiskLevel, SellRecord
...
class ReportGenerator:
    ...
    def _calculate_positive_avg_return(self, portfolio: Portfolio) -> str:
        ...
        if not positive_products:
            return "N/A"
        avg_return = sum(p.annual_return for p in positive_products) / len(positive_products)
    ...
    def get_low_return_products(
        ...
        low_return_products.sort(key=lambda p: p.annual_return, reverse=True)
    ...
    def get_short_term_observation_products(self, portfolio: Portfolio) -> List[Dict[str, Any]]:
        ...
        short_term_products = [
            p for p in portfolio.products
            if p.investment_days and p.investment_days < 90
            and p.annual_return and p.annual_return < Decimal("3")
        ]
        ...
        return [
            {
                ...
                "current_amount": str(p.current_amount) if p.current_amount else "-",
                "status": "短期波动(正常现象)" if p.profit_amount and p.profit_amount < Decimal("0") else "收益偏低(观察)",
            }
    ...
    def get_high_return_reference_products(self, portfolio: Portfolio) -> List[Dict[str, Any]]:
        ...
        high_return_products = [
            p for p in portfolio.products
            if p.annual_return and p.annual_return > Decimal("10")
        ]

        high_return_products.sort(key=lambda p: p.annual_return, reverse=True)
        ...
        return [
            {
                ...
                "profit_amount": str(p.profit_amount) if p.profit_amount else "-",
                "current_amount": str(p.current_amount) if p.current_amount else "-",
                "level": "超高收益" if p.annual_return > Decimal("50") else "高收益",
            }
    ...
    def generate_sold_analysis(self, sell_records: List[SellRecord] = None) -> Optional[Dict[str, Any]]:
        if not sell_records:
            return None
    ...
    def generate_risk_warnings(self, portfolio: Portfolio) -> List[Dict[str, Any]]:
        ...
        if loss_products:
            loss_products.sort(key=lambda p: p.profit_amount)
            warnings.append({
                "level": "danger",
                "type": "loss",
        ...
        long_term_low_return = [
            p for p in portfolio.products
            if p.investment_days and p.investment_days > 365
    ...
    def generate_optimization_suggestions(self, portfolio: Portfolio) -> List[Dict[str, Any]]:
        ...
        transferable_amount = sum(
            float(p.get("current_amount", 0)) for p in low_returns
            if p.get("status") == "收益过低"
        )
        ...
        long_term_low = len([p for p in low_returns if p.get("investment_days") and str(p.get("investment_days")).isdigit() and int(p.get("investment_days")) > 90])
    ...
    def generate_comprehensive_evaluation(
        ...
        if total_initial > Decimal("0"):
            overall_return_rate = (total_profit / total_initial) * 100
        else:
            overall_return_rate = Decimal("0")

        total_investment = total_initial + realized_profit
        ...
        if products_with_days:
            avg_investment_days = Decimal(str(sum(p.investment_days or 0 for p in products_with_days) / len(products_with_days)))
        ...
        if avg_investment_days > Decimal("0") and overall_return_rate > Decimal("0"):
            ...
            if avg_investment_years > Decimal("0"):
                time_weighted_return = ((Decimal("1") + overall_return_rate / Decimal("100")) ** (Decimal("1") / avg_investment_years) - Decimal("1")) * Decimal("100")
            else:
    ...
    def print_console_report(self, report: Dict[str, Any]) -> None:
        ...
        if sold_analysis:
            ...
            sold_table.add_column("指标", style="bold")
            sold_table.add_column("值", style="cyan")
        ...
        if time_analysis.get("groups"):
            ...
            time_table.add_column("分组", style="cyan")
        ...
        portfolio_table.add_row("正收益产品平均年化收益率", f"[yellow]{summary.get('positive_avg_return', 'N/A')}[/yellow]")
                    ...
                    for p in warning["products"][:5]:
                        if isinstance(p, dict):
                            ...
                            if 'return_rate' in p:
                                ...
                                status = p.get('status', '-')
                                console.print(f"  [red]- {name}: 年化 {annual_ret}, 金额 {current_amt}元, {status}[/red]")
                            else:
        ...
        if evaluation:
            ...
            eval_table.add_row("加权年化收益率", f"[green]+{evaluation['weighted_annual_return']}[/green] (按投资金额加权)")
            eval_table.add_row("时间加权年化收益率", f"[green]+{evaluation['time_weighted_return']}[/green] (按平均投资时间{evaluation['avg_investment_days']}天)")
