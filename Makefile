...
north-industry-history: ## 北向资金行业历史数据查询
	@echo "📊 北向资金行业历史数据..."
	$(PY) north-industry --history
...
daily: ## 快速日度分析（更新数据+智能同步股票历史+估算盈亏+自动交易+基金持仓分析+基本面数据更新+ML预测+风险检查+日度报告+数据备份+北向资金+北向资金行业流向）
	...
	@$(MAKE) --no-print-directory north-industry
	@echo ""
	@echo "============================================================"
	@echo "✅ 日度分析完成！"
	@echo "============================================================"
...
backup: ## 备份数据文件
	...
	@test -d data && cp -rL data backup/$(shell date +%Y%m%d)/ 2>/dev/null || cp -r data backup/$(shell date +%Y%m%d)/ 2>/dev/null || true
