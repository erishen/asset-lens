...
daily: ## 快速日度分析（更新数据+智能同步股票历史+估算盈亏+自动交易+基金持仓分析+基本面数据更新+ML预测+风险检查+日度报告+数据备份+北向资金+北向资金行业流向）
	...
	@$(PYTHON) -c "from datetime import datetime; now = datetime.now(); exit(0) if (9 <= now.hour < 15) or (now.hour == 9 and now.minute >= 30) else exit(1)" && echo "⏰ 开市时间，跳过行业流向分析（建议15:00后运行）" || ($(MAKE) --no-print-directory north-industry || echo "⚠️ 北向资金行业数据获取失败，继续执行...")
	@echo ""
	@echo "============================================================"
	@echo "✅ 日度分析完成！"
	@echo "============================================================"
