...
menu: ## 显示交互式菜单
	...
	@echo ""
	@echo "  🎯 快速开始:"
	@echo "    make daily            📊 每日分析（12步完整流程）"
	@echo "    make weekly           📈 生成周报"
	@echo "    make monthly          📅 生成月报"
	@echo "    make analyze          📋 分析投资组合"
	@echo ""
	@echo "  📥 数据初始化:"
	@echo "    make init-data        📦 初始化所有股票数据（首次使用）"
	@echo "    make update           🔄 更新股票数据"
	@echo ""
	@echo "  📊 股票基金查询:"
	...
	@echo ""
	@echo "  🏭 北向资金分析:"
	@echo "    make north-flow       📈 北向资金流向"
	@echo "    make north-industry   🏭 行业流向分析(自动跳过开市时间)"
	@echo "    make north-industry-quick 🏭 行业流向分析(快速模式)"
	@echo "    make north-industry-history 🏭 行业流向历史数据"
	@echo "    make north-industry-force 🏭 行业流向分析(强制获取)"
	@echo ""
	@echo "  🌐 Web 界面:"
...
env-create: ## 创建 conda 环境 asset-lens
	...
	conda create -n asset-lens --clone base -y || true
	@echo "✅ 环境创建成功"
	@echo "💡 请运行: conda activate asset-lens"
...
env-remove: ## 删除 conda 环境 asset-lens
	@echo "🗑️  删除 conda 环境 asset-lens..."
	conda env remove -n asset-lens -y
	@echo "✅ 环境已删除"
...
.PHONY: list-pkgs
list-pkgs: ## 列出已安装的包
	@echo "📋 环境 asset-lens 中已安装的核心包:"
	@uv run pip list | grep -E "pandas|numpy|scipy|click|pydantic|rich|csvkit|pytest" || echo "   ⚠️  未找到相关包"
...
setup: env-create install init init-sample ## 完整初始化（环境+依赖+示例数据）
	...
	@echo ""
	@echo "下一步操作:"
	@echo "  1. 运行 'conda activate asset-lens' 激活环境"
...
.PHONY: north-industry
north-industry: ## 北向资金行业流向分析
	@echo "🏭 北向资金行业流向分析..."
	$(PY) north-industry --save

.PHONY: north-industry-force
north-industry-force: ## 北向资金行业流向分析(强制获取，即使在开市时间)
	@echo "🏭 北向资金行业流向分析(强制模式)..."
	$(PY) north-industry --save --force

.PHONY: north-industry-quick
north-industry-quick: ## 北向资金行业流向分析(快速模式，使用缓存)
	@echo "🏭 北向资金行业流向分析(快速模式)..."
	$(PY) north-industry --skip-fetch

.PHONY: north-industry-history
north-industry-history: ## 北向资金行业流向历史数据
	@echo "🏭 北向资金行业流向历史数据..."
	$(PY) north-industry --history --days 7

.PHONY: north-industry-trend
north-industry-trend: ## 北向资金行业趋势分析（make north-industry-trend INDUSTRY="银行"）
ifndef INDUSTRY
	@echo "❌ 错误: 需要提供行业名称"
	@echo ""
	@echo "用法: make north-industry-trend INDUSTRY=\"银行\""
	@echo ""
	@echo "示例:"
	@echo "  make north-industry-trend INDUSTRY=\"银行\""
	@echo "  make north-industry-trend INDUSTRY=\"半导体\""
	@exit 1
endif
	@echo "📈 $(INDUSTRY) 行业流向趋势..."
	$(PY) north-industry --trend "$(INDUSTRY)"

# ============================================
# 数据初始化与更新
# ============================================
.PHONY: init-data
init-data: ## 初始化所有股票数据（首次使用，下载全部A股历史数据）
	@echo ""
	@echo "============================================================"
	@echo "📦 开始初始化股票数据"
	@echo "============================================================"
	@echo ""
	@echo "⏳ 这将下载所有A股股票的历史数据（约5000只），耗时约1-2小时"
	@echo "   您可以随时按 Ctrl+C 中断，下次会继续下载"
	@echo ""
	@$(MAKE) --no-print-directory db-batch-fetch-all
	@echo ""
	@echo "✅ 股票数据初始化完成！"
	@echo ""

.PHONY: update
update: ## 更新股票数据（智能更新缺失和过期的数据）
	@echo ""
	@echo "============================================================"
	@echo "🔄 开始更新股票数据"
	@echo "============================================================"
...
.PHONY: daily
daily: ## 快速日度分析（更新数据+智能同步股票历史+估算盈亏+自动交易+基金持仓分析+基本面数据更新+ML预测+风险检查+日度报告+数据备份+北向资金+北向资金行业流向）
	@echo ""
	@echo "============================================================"
	@echo "🚀 开始日度分析"
	@echo "============================================================"
	@echo ""
	@echo "📌 步骤 1/12: 📈 更新市场指数数据"
	@$(MAKE) --no-print-directory update-market-data-fast || echo "⚠️ 市场指数数据更新失败（假期/非交易时间可能无数据），继续执行..."
	@echo ""
	@echo "📌 步骤 2/12: 🔄 智能同步股票历史数据"
	@$(MAKE) --no-print-directory db-auto-sync || echo "⚠️ 股票历史数据同步失败，继续执行..."
	@echo ""
	@echo "📌 步骤 3/12: 📊 更新基本面数据（资金流向）"
	@$(MAKE) --no-print-directory fundamental-update || echo "⚠️ 基本面数据更新失败，继续执行..."
	@echo ""
	@echo "📌 步骤 4/12: 💰 估算今日盈亏"
	@$(MAKE) --no-print-directory pnl
	@echo ""
	@echo "📌 步骤 5/12: 🤖 自动交易（模拟盘）"
	@$(MAKE) --no-print-directory auto-trade
	@echo ""
	@echo "📌 步骤 6/12: 📊 基金持仓分析"
	@$(MAKE) --no-print-directory fund-holding
	@echo ""
	@echo "📌 步骤 7/12: 🔮 ML 预测分析"
	...
	@echo ""
	@echo "📌 步骤 11/12: 📈 北向资金分析"
	@$(MAKE) --no-print-directory north-flow
	@echo ""
	@echo "📌 步骤 12/12: 🏭 北向资金行业流向分析"
	@$(MAKE) --no-print-directory north-industry || echo "⚠️ 北向资金行业数据获取失败（非交易时间可能无数据），继续执行..."
	@echo ""
	@echo "============================================================"
	@echo "✅ 日度分析完成！"
	@echo "============================================================"
	...
	@echo ""
	@echo "🏭 北向资金行业分析:"
	@echo "  make north-industry      行业流向分析"
	@echo "  make north-industry-history  历史数据查询"
	@echo "  make north-industry-trend INDUSTRY=\"银行\"  行业趋势"
	@echo ""
...
fundamental-status: ## 查看基本面数据更新状态
	@echo "📊 查看基本面数据更新状态..."
	PYTHONWARNINGS=ignore uv run python -W ignore -m asset_lens.data.update_fundamental_data --mode status

.PHONY: fundamental-update
fundamental-update: ## 每日更新基本面数据（资金流向）
	@echo "📊 每日更新基本面数据..."
	PYTHONWARNINGS=ignore uv run python -W ignore -m asset_lens.data.update_fundamental_data --mode daily

.PHONY: fundamental-update-full
fundamental-update-full: ## 全量更新基本面数据（PE/PB/ROE+资金流向）
	@echo "📊 全量更新基本面数据..."
	PYTHONWARNINGS=ignore uv run python -W ignore -m asset_lens.data.update_fundamental_data --mode full

.PHONY: fundamental-update-incremental
fundamental-update-incremental: ## 增量更新基本面数据
	@echo "📊 增量更新基本面数据..."
	PYTHONWARNINGS=ignore uv run python -W ignore -m asset_lens.data.update_fundamental_data --mode incremental

.PHONY: fundamental-cleanup
fundamental-cleanup: ## 清理过期的基本面数据缓存
	@echo "🧹 清理过期的基本面数据缓存..."
	PYTHONWARNINGS=ignore uv run python -W ignore -m asset_lens.data.update_fundamental_data --mode cleanup --days 30
...
db-batch-fetch: ## 批量获取股票历史数据（make db-batch-fetch LIMIT=100 DAYS=250）
	@echo "📦 批量获取股票历史数据..."
	uv run python scripts/batch_fetch_history.py --limit $(or $(LIMIT),0) --days $(or $(DAYS),250)

.PHONY: db-batch-fetch-all
db-batch-fetch-all: ## 获取所有股票历史数据（耗时较长）
	@echo "📦 获取所有股票历史数据..."
	uv run python scripts/batch_fetch_history.py --days 250
...
test-e2e-fast: ## 运行 E2E 测试（快速模式，跳过慢速测试）
	@echo "⚡ 运行 E2E 测试（快速模式）..."
	@PYTHONWARNINGS=ignore uv run python -W ignore -m pytest tests/e2e/ -v --tb=line -m "not slow"

.PHONY: test-e2e-parallel
test-e2e-parallel: ## 运行 E2E 测试（并行模式，CI 推荐）
	@echo "🚀 运行 E2E 测试（并行模式）..."
	@PYTHONWARNINGS=ignore uv run python -W ignore -m pytest tests/e2e/ -v --tb=line -n auto

.PHONY: test-e2e-optimized
test-e2e-optimized: ## 运行优化的 E2E 测试（合并后的测试）
	@echo "⚡ 运行优化的 E2E 测试..."
	@PYTHONWARNINGS=ignore uv run python -W ignore -m pytest tests/e2e/test_api_optimized.py tests/e2e/test_dashboard_optimized.py -v --tb=short

.PHONY: test-e2e-slow
test-e2e-slow: ## 仅运行慢速 E2E 测试
	@echo "🐌 运行慢速 E2E 测试..."
	@PYTHONWARNINGS=ignore uv run python -W ignore -m pytest tests/e2e/ -v --tb=short -m "slow"

.PHONY: test-e2e-ui
test-e2e-ui: ## 运行 E2E 测试（带 UI 界面）
	@echo "🧪 运行 E2E 测试（UI 模式）..."
	@echo "   请确保 web 服务已启动: make web"
	@PYTHONWARNINGS=ignore uv run python -W ignore -m pytest tests/e2e/ -v --tb=short --headed

.PHONY: test-e2e-debug
test-e2e-debug: ## 运行 E2E 测试（调试模式，慢动作）
	@echo "🧪 运行 E2E 测试（调试模式）..."
	@PYTHONWARNINGS=ignore uv run python -W ignore -m pytest tests/e2e/ -v -s --tb=long --headed --slowmo=500

.PHONY: test-e2e-demo
test-e2e-demo: ## 运行 E2E 演示测试（完整流程演示）
	@echo "🎬 运行 E2E 演示测试..."
	@PYTHONWARNINGS=ignore uv run python -W ignore -m pytest tests/e2e/test_demo.py -v -s --tb=short --headed --slowmo=300

.PHONY: test-e2e-video
test-e2e-video: ## 运行 E2E 测试（录制视频）
	@echo "🎥 运行 E2E 测试（录制视频）..."
	@echo "   视频将保存到 test-results/videos/"
	@RECORD_VIDEO=1 PYTHONWARNINGS=ignore uv run python -W ignore -m pytest tests/e2e/ -v --tb=short --headed

.PHONY: test-e2e-report
test-e2e-report: ## 运行 E2E 测试并生成 HTML 报告
	@echo "📊 运行 E2E 测试并生成报告..."
	@PYTHONWARNINGS=ignore uv run python -W ignore -m pytest tests/e2e/ -v --tb=short --html=test-results/e2e-report.html --self-contained-html
	@echo "📄 报告已生成: test-results/e2e-report.html"

.PHONY: test-e2e-api
test-e2e-api: ## 运行 E2E API 测试（仅 API 相关测试）
	@echo "🧪 运行 E2E API 测试..."
	@PYTHONWARNINGS=ignore uv run python -W ignore -m pytest tests/e2e/test_api_optimized.py -v --tb=short

.PHONY: test-e2e-dashboard
test-e2e-dashboard: ## 运行 E2E Dashboard 测试
	@echo "🧪 运行 E2E Dashboard 测试..."
	@PYTHONWARNINGS=ignore uv run python -W ignore -m pytest tests/e2e/test_dashboard_optimized.py -v --tb=short

.PHONY: test-e2e-failed
test-e2e-failed: ## 运行上次失败的 E2E 测试
	@echo "🔄 运行上次失败的 E2E 测试..."
	@PYTHONWARNINGS=ignore uv run python -W ignore -m pytest tests/e2e/ -v --tb=short --lf
...
test-collect: ## 收集测试用例（诊断用）
	@echo "📋 收集测试用例..."
	@PYTHONWARNINGS=ignore uv run python -W ignore -m pytest tests/ --collect-only
...
status: ## 检查项目状态
	...
	@echo ""
	@echo "  1️⃣  Conda 环境:"
	@conda env list | grep asset-lens || echo "     ⚠️  环境 asset-lens 不存在"
	@echo ""
	@echo "  2️⃣  Python 版本:"
	@$(PY) --version
	@echo ""
	@echo "  3️⃣  核心依赖:"
	@uv run pip list | grep -E "pandas|numpy|scipy|click|pydantic|rich|csvkit" || echo "     ⚠️  某些依赖未安装"
	@echo ""
	@echo "  4️⃣  项目文件:"
...
backup: ## 备份数据库文件
	@echo "💾 备份数据库..."
	@mkdir -p ../investkit_utils/backup/
	@test -f ../investkit_utils/data/asset_lens.db && cp ../investkit_utils/data/asset_lens.db ../investkit_utils/backup/asset_lens.db && echo "✅ 数据库已备份到 investkit_utils/backup/asset_lens.db" || echo "⚠️  数据库文件不存在"
	@test -f ../investkit_utils/data/asset_lens.db-shm && cp ../investkit_utils/data/asset_lens.db-shm ../investkit_utils/backup/ || true
	@test -f ../investkit_utils/data/asset_lens.db-wal && cp ../investkit_utils/data/asset_lens.db-wal ../investkit_utils/backup/ || true
...
sync-data-latest: ## 只同步最新的 ts-demo 数据
	...
	uv run python scripts/sync_data.py --latest
...
fund-holding-bond: ## 分析债券类型基金持仓
	@echo "📊 分析债券类型基金持仓..."
	@PYTHONWARNINGS=ignore uv run python -W ignore scripts/fund_holding_analysis.py --include-bond

.PHONY: fund-list
fund-list: ## 列出所有投资的基金
	@echo "📋 列出所有基金..."
	@uv run python scripts/fund_holding_analysis.py --list
...
ifndef CODE
	...
	@echo "📊 查看基金持仓明细..."
	@uv run python scripts/fund_holding_analysis.py --detail $(CODE)
