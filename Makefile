...
portfolio-metrics: ## 计算投资组合专业指标（夏普比率、最大回撤等）
	@echo "📊 计算投资组合专业指标..."
	$(CONDA) python -m asset_lens analyze

.PHONY: generate-charts
generate-charts: ## 生成投资分析图表（资产配置、风险分布等）
	@echo "📊 生成投资分析图表..."
	$(CONDA) python -m asset_lens chart
...
screen-fundamental: ## 仅基本面筛选
	@echo "📊 基本面筛选..."
	$(CONDA) python -m asset_lens screen-stocks --strategy value

.PHONY: screen-technical
screen-technical: ## 仅技术面筛选
	@echo "📊 技术面筛选..."
	$(CONDA) python -m asset_lens screen-stocks --strategy momentum
...
stock-pool-list: ## 列出股票池中的股票
	@echo "📊 列出股票池..."
	$(CONDA) python -m asset_lens stock-pool
...
strategy-list: ## 列出所有可用策略
	@echo "📊 列出所有可用策略..."
	$(CONDA) python -m asset_lens strategy
...
track-record: ## 记录股票池每日数据
	@echo "📊 记录股票池每日数据..."
	$(CONDA) python -m asset_lens track-stocks
...
market-environment: ## 分析市场环境
	@echo "📊 分析市场环境..."
	$(CONDA) python -m asset_lens market-environment

.PHONY: adapt-strategy
adapt-strategy: ## 适配策略参数（make adapt-strategy STRATEGY=momentum）
	@echo "📊 适配策略参数..."
	$(CONDA) python -m asset_lens strategy --strategy-name $(or $(STRATEGY),momentum)
...
personal-data-load: ## 加载个人每周数据
	@echo "📊 加载个人每周数据..."
	$(CONDA) python -m asset_lens personal-data
...
run-daily-tasks: ## 运行每日任务（立即执行）
	@echo "📊 运行每日任务..."
	$(CONDA) python -m asset_lens run-daily-tasks
...
report-strategy: ## 生成策略报告
	@echo "📊 生成策略报告..."
	$(CONDA) python -m asset_lens strategy --strategy-name momentum
