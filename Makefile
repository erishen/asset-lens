...
test-cov: ## 运行测试并生成覆盖率报告
	@echo "🧪 运行测试并生成覆盖率报告..."
	$(CONDA) pytest --cov=asset_lens --cov-report=html --cov-report=term -n auto
...
web: ## 启动 Web Dashboard (默认端口 8000)
	...
	@echo "📊 Dashboard 地址: http://localhost:8000/dashboard"
	@echo "📚 API 文档地址: http://localhost:8000/docs"
	@echo ""
	$(CONDA) uvicorn asset_lens.web.api:app --host 0.0.0.0 --port 8000 --reload
...
ifndef PORT
	...
	@echo "📊 Dashboard 地址: http://localhost:$(PORT)/dashboard"
	@echo "📚 API 文档地址: http://localhost:$(PORT)/docs"
	@echo ""
	$(CONDA) uvicorn asset_lens.web.api:app --host 0.0.0.0 --port $(PORT) --reload
...
web-bg: ## 后台启动 Web Dashboard
	...
	@echo "📚 API 文档地址: http://localhost:8000/docs"
	@echo ""
	$(CONDA) nohup uvicorn asset_lens.web.api:app --host 0.0.0.0 --port 8000 > web.log 2>&1 &
...
check-commands: ## 自检所有命令是否正常工作
	...
	@echo ""
	@echo "  6️⃣  检查市场数据获取器..."
	@$(PYTHON) -c "from asset_lens.data.enhanced_market_data_fetcher import enhanced_market_data_fetcher" > /dev/null 2>&1 && echo "     ✅ 市场数据获取器正常" || echo "     ❌ 市场数据获取器失败"
	@echo ""
	@echo "  7️⃣  检查股票筛选器..."
	@$(PYTHON) -c "from asset_lens.strategy.screener import StockScreener; StockScreener()" > /dev/null 2>&1 && echo "     ✅ 股票筛选器正常" || echo "     ❌ 股票筛选器失败"
	@echo ""
	@echo "  8️⃣  检查策略引擎..."
	@$(PYTHON) -c "from asset_lens.strategy.engine import StrategyEngine; StrategyEngine()" > /dev/null 2>&1 && echo "     ✅ 策略引擎正常" || echo "     ❌ 策略引擎失败"
	@echo ""
	@echo "  9️⃣  检查 IRR 计算器..."
